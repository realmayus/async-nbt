...
fn main() -> Result<(), NbtIoError> {
    ...
    let nbt = read_nbt(&mut Cursor::new(UNCOMPRESSED_NBT), Flavor::Uncompressed, false)?.0;

    println!("uncompressed: {}", nbt);

    // by changing the Flavor you can read compressed nbt too
    let compressed_nbt = read_nbt(&mut Cursor::new(GZ_COMPRESSED_NBT), Flavor::GzCompressed, false)?.0;
